...
std::vector<std::string> lsdir(const std::string& path) {
    return file_walker_detail::invoke_with_utf8<base_lsdir, given_encoding>(path);
}

template<given_filename_encoding given_encoding>
std::vector<std::string> recursive_lsdir(const std::string& path) {
    return file_walker_detail::invoke_with_utf8<base_recursive_lsdir, given_encoding>(path);
}
...
size_t fs_file_size(const std::string& path) {
    return file_walker_detail::invoke_with_utf8<base_fs_file_size, given_encoding>(path);
}

template<given_filename_encoding given_encoding>
size_t fs_folder_total_size(const std::string& path) {
    return file_walker_detail::invoke_with_utf8<base_fs_folder_total_size, given_encoding>(path);
}
